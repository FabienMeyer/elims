...
class MQTTClient:
    ...
    def _setup_authentication(self) -> None:
        ...
        if self.config.username and self.config.password:
            ...
            msg = MQTTLogMessages.connection_authentication_error(client_type=self.config.client_type, error_msg="No credentials provided")
            logger.error(msg)
    ...
    def _on_connect(self, _client: mqtt.Client | None, _userdata: object | None, flags: dict, rc: int) -> None:
        ...
        if rc == MQTTReturnCode.SUCCESS:
            ...
            self._connection_error = MQTTConnectionError(MQTTLogMessages.connection_failed(self.config.client_type, error_msg))
    ...
    def _on_disconnect(self, _client: mqtt.Client | None, _userdata: object | None, rc: int) -> None:
        ...
        if rc != 0:
            if not self._connection_error:
                self._connection_error = MQTTConnectionError(MQTTLogMessages.unexpected_disconnect(self.config.client_type, rc))
    ...
    def connect(self, timeout: float = 5.0) -> None:
        ...
        if not self._connect_event.wait(timeout):
            ...
            raise MQTTConnectionError(MQTTLogMessages.connection_timeout(self.config.client_type, timeout))
